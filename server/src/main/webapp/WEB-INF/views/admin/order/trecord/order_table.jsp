	...
	<tbody>
		<c:forEach items="${pageInfo.list}" var="list" varStatus="status">
			<tr>
				...
				<td>${list.oDType }</td>
				<td>${list.oMoney }</td>
				<td><c:if test="${list.oStatus==-1}">预约中</c:if> <c:if
						test="${list.oStatus==0}">待商家付定金</c:if> <c:if
				...
				<td><button onclick="editEntity('admin/order/trecord/show', '', '查看佣金', '${list.oId}')" class="btn btn-info">佣金</button></td>
			</tr>
		</c:forEach>
