...
gen_parking_detail = function(data) {
  var imgPlace, parkAddress, parkInfo;
  imgPlace = $('.park_image');
  if (data.images.length) {
    imgPlace.append('<img src="' + data.images[0].otUrl + '" alt=""/>');
  }
  parkInfo = $('.park_info');
  parkInfo.append('<div class="weui-cell"><div class="weui-cell__bd base_info">' + '<div class="title" data-id="' + data.otId + '"><span>' + data.otName + '</span></div>' + '<div class="desc"><span>车位：' + data.otReleaseRes + '</span><span>¥' + data.otPrice + '/小时</span></div>' + '</div>' + '<div class="weui-cell__ft park_info ' + (data.otFreeRes <= 1 ? 'red' : '') + '">' + '剩余：' + data.otFreeRes + '</div>' + '</div>' + '<div class="weui-cell weui-cell_access js_item"><div class="weui-cell__hd address data-lng data-lat"><i class="icon icon-location"/></div><div class="weui-cell__bd address">' + data.otAddress + '</div><div class="weui-cell__ft"></div></div>');
...
gen_map = function() {
  var geolocation, json_data, map, point, pointArray, setTop, top_left_control, top_left_navigation;
  map = '<div class="map_wapper" style="display: none">' + '<div class="map_close">X</div>' + '<div class="map_container">' + '<div id="allmap"></div>' + '</div>' + '</div>';
  $('.wapper').append(map);
  $('.map_wapper').fadeIn();
  $('.back_button').hide();
  ...
  map.addEventListener("click", function(e) {
    return console.log("Click:", e.point.lng, e.point.lat);
  });
  json_data = [
    {
      'point': [113.325939, 23.129881],
      'title': '妇女儿童医疗中心停车场'
    }, {
      'point': [113.327016, 23.131738],
      'title': '津滨腾越停车场'
    }
  ];
...
parking_action = function() {
  ...
  msgLoading = gen_msg_loading();
  return place_order(true, params, place_order_success_cb, wx_pay_success_cb);
};
...
order_bill_action = function() {
  ...
  return place_order(need_pay !== 0, params, place_order_success_cb, wx_pay_success_cb);
};
...
place_order = function(need_pay, params, po_success_cb, wp_success_cb) {
  if (need_pay == null) {
    need_pay = true;
  }
  return $.ajax({
...
check_user_order_status = function() {
  var checked_callback, existed, msgLoading, params, token;
  existed = false;
  checked_callback = function(data) {
    ...
    if (data) {
      ...
      if (status === 1) {
        existed = true;
        if (data.odAddPrice === data.odFirstPrice) {
...
query_order_info = function() {
  return underway_order(gen_order_info);
};

gen_order_info = function(order) {
  var $detail, $order, $park, $parkAddress, end_time, leave_time, need_pay, park, unit_price;
  if (!order) {
    alert('未查询到相关信息');
    history.go(-1);
    return;
...
underway_order = function(callback) {
  var msgLoading, params, token;
  msgLoading = gen_msg_loading();
  token = get_token();
  params = {
...
query_bill_info = function() {
  return underway_order(gen_bill_info);
};
...
$.Dialog = function(config, type) {
  ...
  self.init = function() {
    ...
    if ($('.dialog_' + type).length === 0) {
      dialog = '<div class="dialog_' + type + '" style="display: none;">' + '<div class="weui-mask" style="z-index: 1000;"></div>' + '<div class="weui-dialog" ' + (type === 1 ? 'style="position: fixed; top:20%"' : '') + '>' + (type === 1 ? '<div class="weui-dialog__hd"><strong class="weui-dialog__title">' + config.title + '</strong></div>' : '') + '<div class="weui-dialog__bd">' + config.content + '</div>' + '<div class="weui-dialog__ft">' + (type === 1 ? '<a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_default dialog_auxiliary">' + auxiliary.text + '</a>' : '') + '<a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary dialog_main">' + main.text + '</a>' + '</div>' + '</div>' + '</div>';
      $('body').append(dialog);
      $element = $('.dialog_' + type);
      $(document).on('touchend', '.dialog_' + type + ' .dialog_auxiliary', function() {
        var is_close;
        is_close = true;
      ...
      return $(document).on('touchend', '.dialog_' + type + ' .dialog_main', function() {
        var is_close;
        is_close = true;
        if (main.callback) {
